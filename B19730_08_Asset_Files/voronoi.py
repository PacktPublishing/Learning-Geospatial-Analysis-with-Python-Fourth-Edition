...
#        return (lid,rid)
    ...
    def outVertex(self,s):
        ...
        if self.xmin is None or s.x < self.xmin:
            self.xmin = s.x
        elif self.xmax is None or s.x > self.xmax:
            self.xmax = s.x
        if self.ymin is None or s.y < self.ymin:
            self.ymin = s.y
        elif self.ymax is None or s.y > self.ymax:
            self.ymax = s.y
...
class SiteList(object):
    def __init__(self,pointList):
        ...
        self.__sites.sort(key=lambda s: s.y)
#        self.__sites.sort()
