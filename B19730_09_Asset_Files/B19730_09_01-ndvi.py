...
# https://github.com/PacktPublishing/Learning-Geospatial-Analysis-with-Python-4th-Edition/raw/main/B19730_09_Asset_Files/NDVI.zip
...
# and silence gdal warnings
gdal.PushErrorHandler('CPLQuietErrorHandler')
gdal.UseExceptions()

def imageToArray(i):
    #a = gdal_array.numpy.frombuffer(i.tobytes(), dtype=np.uint8)
...
rClip = np.choose(mask, (rClip, 0)).astype(float)

# Clip the infrared band using the mask
irClip = np.choose(mask, (irClip, 0)).astype(float)
...
# Set small threshold value
epsilon = 1e-5  
# NDVI calculation
ndvi = (irClip - rClip) / (irClip + rClip + epsilon).astype(gdal_array.numpy.float32)


# Normalize NDVI values to be between 0 and 1
#normalized_ndvi = (ndvi + 256) / 2.0

# Remove any NaN values from the final product
#normalized_ndvi = gdal_array.numpy.nan_to_num(normalized_ndvi)
